...
void main() {
  ...
  // Organic noise field
  float n1 = fbm(vec3(uv * 2.0, t));
  float n2 = fbm(vec3(uv * 2.0 + 5.0, t + 10.0));

  // Mix colors based on noise
  vec3 color = mix(u_color1, u_color2, smoothstep(-0.5, 0.5, n1));
  color = mix(color, u_color3, smoothstep(-0.3, 0.6, n2));
  color = mix(color, u_color4, smoothstep(0.2, 0.8, n1 * n2 + 0.5));
